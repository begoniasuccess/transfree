<template>
  <div class="h_100 w_100">
    <div class="flex_row_cb w_100 h_100">
      ...
      <!--              </div>-->
      <!--            </div>-->
      <SearchList :selectedCity="selected.value"></SearchList>
...
export default {
  name: "SearchBus",
  components: { DynamicKeyboard, BusInfo, SearchList },
  data() {
    return {
      cities: CITIES,
      selected: CITIES[0],
  ...
  mounted() {
    ...
    //   `${BUS_URL_V2}/Route/City/${this.selected.value}?$top=${DEFAULT_NUMBER_OF_QUERY_RECORDS}&$format=${RESPONSE_DATA_FORMAT_JSON}`
    // ).then((res) => {
